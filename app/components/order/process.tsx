...
const processingOrdersData: OrderItem[] = [
  {
    id: "ORD-1052",
    itemsCount: 3,
...
export default function ProcessingOrdersPage() {
  ...
  const filteredOrders = useMemo(() => {
    return processingOrdersData.filter((order) => {
      const matchesSearch =
        search.trim() === "" ||
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `processing_orders_${Date.now()}.csv`);
              ...
              {filteredOrders.length === 0 ? (
                <tr>
                  <td colSpan={8} className="py-12 text-center text-slate-400 font-semibold">
                    No processing orders found matching criteria.
                  </td>
                </tr>
              ) : (
                filteredOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    {/* Order Status Badge (Soft Orange Processing Pill) */}
                    <td className="py-4 px-4 text-right">
                      <span className="px-3 py-1 rounded-full bg-[#ffedd5] text-[#ea580c] font-extrabold text-[10px]">
                        Processing
                      </span>
                    </td>
