...
const packedOrdersData: OrderItem[] = [
  {
    id: "ORD-1052",
    itemsCount: 3,
...
export default function PackedOrdersPage() {
  ...
  const filteredOrders = useMemo(() => {
    return packedOrdersData.filter((order) => {
      const matchesSearch =
        search.trim() === "" ||
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `packed_orders_${Date.now()}.csv`);
              ...
              {filteredOrders.length === 0 ? (
                <tr>
                  <td colSpan={8} className="py-12 text-center text-slate-400 font-semibold">
                    No packed orders found matching criteria.
                  </td>
                </tr>
              ) : (
                filteredOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    {/* Order Status Badge (Exact Light Yellow/Amber Packed Pill) */}
                    <td className="py-4 px-4 text-right">
                      <span className="px-3 py-1 rounded-full bg-[#fef3c7] text-[#d97706] font-extrabold text-[10px]">
                        Packed
                      </span>
                    </td>
