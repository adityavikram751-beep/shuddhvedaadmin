...
const lowStockOrdersData: OrderItem[] = [
  {
    id: "ORD-1052",
    itemsCount: 3,
...
export default function LowStockOrdersPage() {
  ...
  const filteredOrders = useMemo(() => {
    return lowStockOrdersData.filter((order) => {
      const matchesSearch =
        search.trim() === "" ||
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `low_stock_orders_${Date.now()}.csv`);
              ...
              {filteredOrders.length === 0 ? (
                <tr>
                  <td colSpan={8} className="py-12 text-center text-slate-400 font-semibold">
                    No low stock orders found matching criteria.
                  </td>
                </tr>
              ) : (
                filteredOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    {/* Order Status Badge (Exact Low Stock Pill) */}
                    <td className="py-4 px-4 text-right">
                      <span className="px-3 py-1 rounded-full bg-[#fff3e6] text-[#d9730d] font-extrabold text-[10px] tracking-wide uppercase">
                        LOW STOCK
                      </span>
                    </td>
