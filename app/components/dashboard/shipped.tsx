...
const shippedOrdersData: OrderItem[] = [
  {
    id: "ORD-1052",
    itemsCount: 3,
...
export default function ShippedOrdersPage() {
  ...
  const filteredOrders = useMemo(() => {
    return shippedOrdersData.filter((order) => {
      const matchesSearch =
        search.trim() === "" ||
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `shipped_orders_${Date.now()}.csv`);
              ...
              {filteredOrders.length === 0 ? (
                <tr>
                  <td colSpan={8} className="py-12 text-center text-slate-400 font-semibold">
                    No shipped orders found matching criteria.
                  </td>
                </tr>
              ) : (
                filteredOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    {/* Order Status (Soft Purple Pill for Shipped) */}
                    <td className="py-4 px-4 text-right">
                      <span className="px-3.5 py-1 rounded-full bg-[#f3e8ff] text-[#9333ea] font-extrabold text-[10px]">
                        Shipped
                      </span>
                    </td>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-100 text-xs text-slate-500">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="p-1.5 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-slate-600 transition-colors"
            >
              <ChevronLeft size={15} />
            </button>
            ...
            <button
              ...
              className="w-7 h-7 rounded-xl bg-[#d9730d] text-white font-extrabold text-xs flex items-center justify-center shadow-xs"
            >
              1
            </button>
            ...
            <button
              ...
              className="w-7 h-7 rounded-xl bg-white border border-slate-200 text-slate-700 font-bold text-xs hover:bg-slate-50 flex items-center justify-center"
            >
              2
            </button>
